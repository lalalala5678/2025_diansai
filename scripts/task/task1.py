...
pulse1_min = 1100    # 水平舵机最小脉宽 (激光点位于右边界)
pulse1_max = 1900    # 水平舵机最大脉宽 (激光点位于左边界)
pulse2_min = 1300    # 垂直舵机最小脉宽 (激光点位于上边界)
pulse2_max = 1700    # 垂直舵机最大脉宽 (激光点位于下边界)

# 设置摄像头
picam2 = Picamera2()
# 创建全分辨率still配置 (假定相机模块支持2592x2592分辨率)
...
def move_servos(pulse1, pulse2):
    ...
    try:
        ServoControl.setPWMServoMove(1, int(pulse1), 500)
        ServoControl.setPWMServoMove(2, int(pulse2), 500)
...
def capture_and_detect(num_frames=5):
    ...
    for i in range(num_frames):
        ...
        result = detect_laser.detect_green(frame, M=None, debug=False)  # M为透视矩阵，若无则传None
...
quarter_pulse = pulse1_max - 0.25 * (pulse1_max - pulse1_min)
three_quarter_pulse = pulse1_max - 0.75 * (pulse1_max - pulse1_min)
...
if quarter_frame and three_quarter_frame:
    ...
    logging.info(f"水平1/4位置逻辑(校正后) = {q_logic_x_adj:.2f}, 3/4位置逻辑(校正后) = {tq_logic_x_adj:.2f}")
    # 计算相对于中心50的差值
    ...
    for _ in range(30):  # 迭代30次足够精度
        ...
        if ratio > target_ratio:
            ...
            low = mid
        angle_h_est = mid
    angle_h_calibrated = angle_h_est if angle_h_est is not None else (abs(pulse1_max - pulse1_min) * 0.1)
    logging.info(f"拟合得到水平最大扫描角度 angle_h ≈ {angle_h_calibrated:.2f} 度")
else:
    # 若中间点检测不足，则根据舵机脉宽范围粗略估计角度范围
    angle_h_calibrated = abs(pulse1_max - pulse1_min) * 0.1
...
# 中心（水平中心，垂直中心已在上面移动过）
if center_frame is None:
    # 若之前中心没取到，这里尝试获取一次中心位置
    move_servos(pulse1_center, pulse2_center)
    time.sleep(0.3)
    center_frame = capture_and_detect()
    if center_frame is None:
        logging.warning("中心位置(垂直)激光点再次未检测到，继续标定")
        center_x_pixel_v, center_y_pixel_v = None, None
    else:
...
quarter_pulse_v = pulse2_max - 0.25 * (pulse2_max - pulse2_min)
three_quarter_pulse_v = pulse2_max - 0.75 * (pulse2_max - pulse2_min)
...
if quarter_v_frame and three_quarter_v_frame:
    ...
    logging.info(f"垂直1/4位置逻辑(校正后) = {q_logic_y_adj:.2f}, 3/4位置逻辑(校正后) = {tq_logic_y_adj:.2f}")
    ...
    for _ in range(30):
        ...
        if ratio > target_ratio_v:
            ...
            low = mid
        angle_v_est = mid
    angle_v_calibrated = angle_v_est if angle_v_est is not None else (abs(pulse2_max - pulse2_min) * 0.1)
    logging.info(f"拟合得到垂直最大扫描角度 angle_v ≈ {angle_v_calibrated:.2f} 度")
else:
    angle_v_calibrated = abs(pulse2_max - pulse2_min) * 0.1
